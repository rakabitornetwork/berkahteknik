...
export default function Index({ items }) {
    const handleDelete = (id) => {
        if (confirm('Yakin ingin menghapus data jasa ini?')) {
            router.delete(`/admin/work-types/${id}`);
        }
    };

    return (
        <AdminLayout title="Master Data · Data Jasa">
            <Head title="Data Jasa" />
            <div style={{ padding: '0.25rem 0' }}>
                <MasterDataTabs />
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', gap: '1rem', flexWrap: 'wrap' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem' }}>
                        <ClipboardList size={22} style={{ color: 'var(--color-primary)' }} />
                        <h2 style={{ margin: 0, fontSize: '1.25rem', fontWeight: 700 }}>Data Jasa</h2>
                    </div>
                    <Link href="/admin/work-types/create" className="btn btn-primary" style={{ display: 'inline-flex', alignItems: 'center', gap: '0.4rem', fontSize: '0.875rem' }}>
                                ...
                                <tr key={item.id}>
                                    ...
                                    <td style={{ textAlign: 'center' }}>
                                        <div style={{ display: 'flex', justifyContent: 'center', gap: '0.5rem' }}>
                                            <Link href={`/admin/work-types/${item.id}/edit`} style={{ color: 'var(--color-primary)' }}><Edit size={16} /></Link>
